...
document.addEventListener('DOMContentLoaded', function () {
    const station5Container = document.getElementById('station5');
    if (!station5Container) return;

    // 获取元素
    const slider = document.getElementById('miracle-slider');
    const islandImage = document.getElementById('palm-island-image');
    const progressText = document.getElementById('s5-progress-text');

    // --- 新增：获取全局导航按钮 ---
    const globalNav = document.querySelector('.global-nav');

    const progressCommentary = [
        "一切始于一片汪洋...", // 0-19%
        "通过GPS精确定位，700万吨岩石构筑起新月形的防波堤，为奇迹奠定基石。", // 20-39%
        "9400万立方米的海沙被从海底吹填而上，开始勾勒出棕榈树的雏形。", // 40-59%
        "17片“棕榈叶”逐渐伸展，每一片都代表着一座未来的奢华社区。", // 60-79%
        "岛屿的轮廓已清晰可见，一个挑战自然极限的宏伟蓝图即将完成。", // 80-99%
        "没有一颗钢筋，全靠9400万立方米海沙和700万吨岩石。<br>这是贝都因人对‘不可能’这个词的复仇。" // 100%
    ];
    ...
    slider.addEventListener('input', function () {
        ...
        islandImage.style.clipPath = `inset(0 ${clipPercentage}% 0 0)`;

        let commentaryIndex = Math.floor(value / 20);
        if (value === 100) {
            commentaryIndex = 5;
        }

        if (progressText.innerHTML !== progressCommentary[commentaryIndex]) {
            progressText.style.opacity = 0;
            setTimeout(() => {
                progressText.innerHTML = progressCommentary[commentaryIndex];
                progressText.style.opacity = 1;
            }, 300);
        }
    });

    // --- 升级版的 Intersection Observer ---
    const observerS5 = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // 当进入第五站时
                slider.value = 0;
    ...
    observerS5.observe(station5Container);
});
